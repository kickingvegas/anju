...
:PROPERTIES:
:CUSTOM_ID: org-mode-context-menu
:END:
...
On an existing Org link, use the menu item “Copy Link Address…” to copy the raw link address into the clipboard (~kill-ring~).

[[file:images/anju-context-menu-org-link-copy-address.png]]
...
*** Org Agenda Context Menu
:PROPERTIES:
:CUSTOM_ID: org-agenda-context-menu
:END:
#+CINDEX: Org Agenda Context Menu

[[file:images/anju-context-menu-org-agenda.png]]

Anju offers a context menu for Org Agenda ([[info:org#Agenda Views]]) providing commands that support updating a headline or changing the view of headlines (tasks) displayed.


*** Dired Mode Context Menu
:PROPERTIES:
:CUSTOM_ID: dired-mode-context-menu
:END:
...
:PROPERTIES:
:CUSTOM_ID: emacs-lisp-context-menu
:END:
...
:PROPERTIES:
:CUSTOM_ID: compilation-context-menu
:END:
...
*** Xref Context menu
:PROPERTIES:
:CUSTOM_ID: xref-context-menu
:END:
#+CINDEX: Xref Context Menu


[[file:images/anju-context-menu-xref.png]]

Commands such as ~xref-find-references~ or ~xref-find-references-at-mouse~ on a symbol will create a new buffer named ~✻xref✻~ listing locations where that symbol has been referenced. Anju provides a context menu for this  ~✻xref✻~  buffer with commands described in [[info:emacs#Xref Commands]].


*** Makefile Context Menu
:PROPERTIES:
:CUSTOM_ID: makefile-context-menu
:END:
...
:PROPERTIES:
:CUSTOM_ID: rectangle-context-menu
:END:
...
*** Version-Controlled Source
:PROPERTIES:
:CUSTOM_ID: version-controlled-source
:END:
...
*** Registers Menu Features
#+CINDEX: Registers Menu Features

[[file:images/anju-main-menu-registers.png]]


Anju adds a *Registers* menu to the menu bar to work with Emacs registers ([[info:emacs#Registers]]). Registers offer run-time storage of Emacs state such as:
...
This modification is optional. For more detail, see [[#registers-menu-customization][Registers Menu Customization]].
...
Anju adds a *Bookmarks* menu to the menu bar to work with Emacs-specific bookmarks.
...
[[file:images/anju-kmacro-menu-recording.png]]
...
[[file:images/anju-kmacro-menu.png]]
...
A context menu function has two arguments:
  ...
  - ~click~ : a mouse event which holds the position of the raised context menu.
...
The predicate logic implementation can take place at different levels:
  ...
  1. At the time of menu item instantiation.
     ...
     If the menu item is instantiated using the macro ~easy-menu-define~, then the ~:visible~ and ~:active~ keywords can control how the menu item is displayed ([[info:elisp#Easy Menu]]).
...
#+TEXINFO: @subheading Context Menu Current Point Handling
#+CINDEX: Context Menu Current Point Handling
...
- The context menu contains commands that only apply to a major mode. This permits implicit mutual exclusion between major modes.
- The context menu contains a command which depends on the current point.
- The current point is moved only if /p/ and /p'/ are distant.
...
It is strongly recommended to use the ~customize-variable~ command to configure ~context-menu-functions~ for the following reasons:
...
- Users can insert their own custom context menu function.

- Top-down ordering of functions will determine the order of menu items in the context menu.

Users who insist on using ~add-hook~ to modify ~context-menu-functions~ should be mindful of its function ordering.
...
#+VINDEX: anju-context-menu-org-agenda
#+VINDEX: anju-context-menu-info-mode
...
- ~anju-context-menu-dired~ :: Dired commands. See [[#dired-mode-context-menu][Dired Mode Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-org-mode~  :: Org mode commands. See [[#org-mode-context-menu][Org Mode Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-org-agenda~  :: Org Agenda commands. See [[#org-agenda-context-menu][Org Agenda Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-info-mode~  :: Info mode commands. See [[#info-context-menu][Info Context Menu]].

- ~anju-context-menu-make-mode~  :: Makefile mode commands. See [[#makefile-context-menu][Makefile Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-compile~  :: Compilation (or Grep) mode commands. See [[#compilation-context-menu][Compilation/Grep Context Menu]].

- ~anju-context-menu-elisp~ :: Emacs Lisp commands. See [[#emacs-lisp-context-menu][Emacs Lisp Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-edebug-eval~ :: Edebug commands. See [[#edebug-mode-context-menu][Edebug Mode Context Menu]].

  This function changes the current point to where the context menu is raised.

- ~anju-context-menu-xref~ :: Xref commands. See [[#xref-context-menu][Xref Context Menu]].
...
- ~anju-context-menu-region-extension~ :: Commands for extending the menu items provided by the built-in ~context-menu-region~ hook.
...
- ~anju-context-menu-vc~ :: Version control related commands.
  ...
  This function changes the current point to where the context menu is raised. If Magit 4.4+ is installed, then calls to it are made from here. See [[#version-controlled-source][Version-Controlled Source]].
...
- ~anju-context-menu-rectangle~ :: Rectangle commands. See [[#rectangle-context-menu][Rectangle Context Menu]].
...
*** Registers Menu Customization
:PROPERTIES:
:CUSTOM_ID: registers-menu-customization
:END:
#+CINDEX: Registers Menu Customization
#+VINDEX: anju-main-menu--reconfigure-registers

Anju customizes the *Registers* menu using the hook function ~anju-main-menu--reconfigure-registers~.

To add the *Registers* menu to the menu bar, add the above hook function to ~anju-reconfigure-main-menu-hook~.

If ~customize-variable~ is used to configure ~anju-reconfigure-main-menu-hook~, then a checkbox interface to enable  ~anju-main-menu--reconfigure-registers~ is displayed.
